...
/**
 * This is the utility class for all servlets. It provides method for inserting, deleting, searching the entity from data store. Also contains method for
...
public class Util {
     ...
     */
    public static Iterable<Entity> listEntities(String kind, String searchBy, Object searchFor, String searchBy2, Object searchFor2, String orderBy) {
	...
	if (searchFor2 != null && !"".equals(searchFor2)) {
	    q.addFilter(searchBy2, FilterOperator.EQUAL, searchFor2);
	}
	if (orderBy != null && !"".equals(orderBy)) {
	    q.addSort(orderBy, SortDirection.ASCENDING);
	}
	...
	for (Entity result : entities) {
	    ...
	    Entity work = findEntity((Key) result.getProperty(fkName));
	    for (String key : work.getProperties().keySet()) {
		sb.append("\"" + key + "\" : \"" + work.getProperties().get(key) + "\",");
	    }
	    sb.deleteCharAt(sb.lastIndexOf(","));
	    sb.append("},");
	    i++;
     ...
     * Retrieves Parent and Child entities into JSON String
     * @param entities : List of parent entities
     * @param fkName : foreign-key to the child in the child entity
     * @param fkEntityMaxSize : int
     * @return JSON string
     */
    public static String writeFragmentJSON(Iterable<Entity> entities, String fkName, int fkEntityMaxSize) {
	logger.log(Level.INFO, "creating JSON format object for parent child relation");
	StringBuilder sb = new StringBuilder();
	int i = 0;
	sb.append("{\"data\": [");
	for (Entity result : entities) {
	    ...
	    for (int j = 0; j < fkEntityMaxSize; j++) {

		Map<String, Object> properties = result.getProperties();

		Key fk = (Key) result.getProperty(fkName + j);

		if (fk != null) {
		    sb.append("{");
		    sb.append("\"seqTime\" : \"" + properties.get("seqTime" + j) + "\",");
		    sb.append("\"seqType\" : \"" + properties.get("seqType") + "\",");
		    Entity work = findEntity(fk);
		    for (String key : work.getProperties().keySet()) {
			sb.append("\"" + key + "\" : \"" + work.getProperties().get(key) + "\",");
		    }
		    sb.deleteCharAt(sb.lastIndexOf(","));
